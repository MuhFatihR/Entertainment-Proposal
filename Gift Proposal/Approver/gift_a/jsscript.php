<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        table = $('#GiftTable').DataTable({
            ...
            "ajax": {
                ...
                "data": function(d) {
                    ...
                    // Tambahkan useremail ke data yang dikirim
        ...
        $('#GiftTable').on('click', '.showDetail', function() {
            var id = $(this).data('id');
            var idApprover = $(this).data('idapprover'); // Tidak perlu `data-` di depan
            console.log("ID Approver:", idApprover); // Cetak ke konsol
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data.dataApprover != null && data.dataApprover.length > 0) { // Validasi Button Accept & Reject based on status_pengajuan user
                        let selectedApprover = data.dataApprover.find(e => e.id === id_approver); // Find the selected approver
                        let selectedApproverLevel = selectedApprover?.approver_level; // Get the seleted "Approver Level"

                        // Cek apakah ada approver dengan level lebih kecil dan status 'Waiting Approval'
